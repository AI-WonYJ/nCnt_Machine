...
@app.get('/')
async def Web_delivery(request: Request, ncnt_people: str, current_time: str, standard_time: str):
    machine()  # machine 작동
    current_time = datetime.now()  # 실시간 시간 측정
    current_time = str(current_time)[0:19]  # 필요한 부분 가공
    return templates.TemplateResponse('new.html', {"request": request, "counting": ncnt_people, "time": current_time, "old_time": standard_time})  # Flask로 new.html에 변수 값 전달
